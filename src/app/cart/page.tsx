...
export default function CartPage() {
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-10 md:py-16">
      <h1 className="font-serif text-4xl md:text-5xl font-medium text-charcoal mb-10">
        Your Cart
      </h1>
      ...
      {items.length === 0 ? (
        ...
        <>
          {/* Header row */}
          <div className="hidden md:grid md:grid-cols-[1fr_120px_120px_40px] gap-4 pb-4 border-b border-border text-xs text-warm-gray uppercase tracking-wide">
            <span>Product</span>
          ...
          <ul className="divide-y divide-border">
            {items.map((item) => (
              ...
              >
                ...
                <div className="flex items-center justify-center col-start-2 md:col-start-auto">
                  <div className="flex items-center border border-border">
                    <button
                      ...
                      className="px-2.5 py-1.5 text-sm text-charcoal hover:text-terracotta transition-colors"
                      aria-label={`Decrease quantity of ${item.product.title}`}
                    >
                      &minus;
                    </button>
                    <span className="px-3 py-1.5 text-sm text-charcoal min-w-[32px] text-center border-x border-border">
                      {item.quantity}
                    </span>
                    <button
                      onClick={() =>
                ...
                {/* Line total */}
                <p className="hidden md:block text-sm text-charcoal text-right">
                ...
                <button
                  ...
                  className="hidden md:block text-warm-gray hover:text-terracotta transition-colors duration-200"
          ...
          <div className="border-t border-border mt-2 pt-8">
            <div className="flex flex-col items-end gap-4">
              ...
              <Link
                href="/shop"
                className="text-sm text-charcoal hover:text-terracotta transition-colors duration-200"
              >
                Continue Shopping
              </Link>
            </div>
